...
INormal_Uniform(mu, sigma, mu_sup) = INormal_Uniform(Float64(mu), Float64(sigma), Float64(mu_sup), Float64(sigma))
...
Distributions.logpdf(d::INormal_Uniform, x::Real) = Distributions.logpdf(Normal(d.mu, d.sigma), x)# - Distributions.logpdf(Uniform(d.mu_sup-d.sup/2, d.mu_sup+d.sup/2), x)
