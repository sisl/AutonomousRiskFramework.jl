...
def main():
    ...
    argparser.add_argument(
        ...
        metavar='SPEC_LOC',
        nargs="+",
        help='positions a spectator at a point in the map')
    if len(sys.argv) < 2:
    ...
    if args.spectator_loc is not None:
        assert(len(args.spectator_loc)==3)
        spectator = world.get_spectator()
        new_location = carla.Location(x=float(args.spectator_loc[0]), y=float(args.spectator_loc[1]), z=50+float(args.spectator_loc[2]))
