...
class ScenarioRunner(object):
    ...
    def _load_scenario(self, config):
        ...
        try:
            ...
            start_time = self.manager.prep_scenario_ast()
    ...
    def _stop_scenario(self, start_time, recorder_name, scenario):
        try:
            self.manager.clean_scenario_ast(start_time)
    ...
    def _run_route(self):
        ...
        result = self.run_AST(route_configurations)
        return result

    def run_AST(self, route_configurations):
        temp_record = self._args.record

        self._args.record = False

        self.temp_scenario = {'count': 0, 'start_time': None, 
                                'recorder_name': None, 'scenario': None}

        config = route_configurations[0]
        block_size = 10
        self.running = False

        def reset_fn():
            if self.running:
                result = self._stop_scenario(self.temp_scenario['start_time'], self.temp_scenario['recorder_name'], self.temp_scenario['scenario'])
                self._cleanup()
            ...
            self.temp_scenario['scenario'] = scenario
        ...
        def step_fn(values):
            ...
            if not self.running:
                result = self._stop_scenario(self.temp_scenario['start_time'], self.temp_scenario['recorder_name'], self.temp_scenario['scenario'])
                self._cleanup()
            return self.running, failures, distance
