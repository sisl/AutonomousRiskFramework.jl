...
function scenario_hw_merging(; init_noise_1=Noise(), init_noise_2=Noise())
    ...
    sut = BlinkerVehicleAgent(hw_straight(id=1, s=s1, noise=init_noise_1, roadway=roadway), sut_model);
    ...
    adversary = BlinkerVehicleAgent(hw_merging(id=2, s=s2, noise=init_noise_2, roadway=roadway), adversary_model)
...
# TODO: (kwargs...) for `t_intersection`
# TODO: UrbanIDM???
function scenario_hw_merging_no_obs(; init_noise_1=Noise(), init_noise_2=Noise(), s1::Float64=3.0, s2::Float64=9.0)
    roadway = merging_roadway()

    # sut_model = UrbanIDM(idm=IntelligentDriverModel(v_des=15.0), noisy_observations=true, ignore_idm=!params.ignore_sensors)
    sut_model = TIDM(get_XIDM_template())
    sut_model.idm.v_des = 15
    sut_model.noisy_observations = false # TODO. Flip who is SUT/adversary?
    sut_model.goals[3] = [3]
    map(k->delete!(sut_model.goals, k), 4:7)
    sut = BlinkerVehicleAgent(hw_straight(id=1, s=s1, noise=init_noise_1, roadway=roadway), sut_model);
    ...
    adversary_model = TIDM(get_XIDM_template())
    adversary_model.idm.v_des = 15
    adversary_model.noisy_observations = false # TODO: important?
    adversary_model.goals[3] = [3]
    map(k->delete!(adversary_model.goals, k), 4:7)
    adversary = BlinkerVehicleAgent(hw_merging(id=2, s=s2, noise=init_noise_2, roadway=roadway), adversary_model)
