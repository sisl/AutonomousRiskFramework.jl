...
function ISDPWSolver(;depth::Int=10,
                    ...
                    timer=() -> 1e-9*time_ns())
        ISDPWSolver(depth, exploration_constant, n_iterations, max_time, k_action, alpha_action, k_state, alpha_state, virtual_loss,
                    keep_tree, enable_action_pw, enable_state_pw, check_repeat_state, check_repeat_action, tree_in_info, rng,
                    estimate_value, init_Q, init_N, next_action, default_action, reset_callback, show_progress)
end


mutable struct DPWActionNode{S,A}
    id::Int
    a_label::A
    n::Int
    q::Float64
...
DPWActionNode(id::Int, a::A, n::Int, q::Float64) where {S,A} =
    DPWActionNode{S,A}(id, a, n, q, Vector{Tuple{S,Float64}}[], Set{S}(), 0, ReentrantLock())

@inline n(n::DPWActionNode) = n.n
@inline q(n::DPWActionNode) = n.q
@inline n_a_children(n::DPWActionNode) = n.n_a_children
...
mutable struct DPWStateNode{S,A}
    ...
    s_label::S
    total_n::Int
    children::Vector{DPWActionNode{S,A}}
    s_lock::ReentrantLock
    # Action nodes currently being evaluated. Used for applying virtual loss.
    a_selected::Set{A}
end
DPWStateNode(id::Int, s::S, total_n::Int) where {S,A} =
    DPWStateNode{S,A}(id, s, total_n, Vector{DPWActionNode{S,A}}(), ReentrantLock(), Set{A}())

@inline children(n::DPWStateNode) = n.children
@inline n_children(n::DPWStateNode) = length(children(n))
@inline isroot(n::DPWStateNode) = (n.id == 1)


mutable struct PDPWTree{S,A}
    root::Union{Nothing, S}

    state_nodes::Dict{S, DPWStateNode}
    state_action_nodes::Dict{Tuple{S,A}, DPWActionNode}
    ...
    function PDPWTree{S,A}(root::Union{Nothing, S}=nothing) where {S,A} 
        return new(root,
                   Dict{S, DPWStateNode}(),
                   Dict{Tuple{S,A}, DPWActionNode}(),
...
Base.isempty(tree::PDPWTree) = isempty(tree.state_nodes)


function insert_state_node!(tree::PDPWTree{S,A}, s::S) where {S,A}
    haskey(tree.state_nodes, s) && return tree.state_nodes[s]
    id = Threads.atomic_add!(tree._s_id_counter, 1)
    snode = DPWStateNode(id, s, 0)
    tree.state_nodes[s] = snode
    return snode
end


function insert_action_node!(tree::PDPWTree{S,A}, snode::DPWStateNode, a::A, n0::Int, q0::Float64) where {S,A}
    ...
    sanode = DPWActionNode(id, a, n0, q0)
    tree.state_action_nodes[state_action_key] = sanode
    push!(snode.children, sanode)
    snode.total_n += n0
    return sanode
end
...
mutable struct ISDPWPlanner{P<:Union{MDP,POMDP}, S, A, SE, NA, RCB, RNG} <: AbstractMCTSPlanner{P}
    solver::ISDPWSolver
    mdp::P
    tree::Union{Nothing, PDPWTree{S,A}}
    solved_estimate::SE
    next_action::NA
    reset_callback::RCB
    rng::RNG
